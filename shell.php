...
?>
<?php
	$wicd_root=$_SERVER['DOCUMENT_ROOT'] . str_replace('shell.php', '', strtok($_SERVER['REQUEST_URI'], '?'));
	if(isset($_GET['wicd']))
		switch($_GET['wicd'])
		{
			// settings
			case 'setWifiDevice':
				echo shell_exec($wicd_root . 'shell.sh wicd setWifiDevice ' . $_GET['device']);
				exit();
			break;
			case 'getWifiDevice':
			...
			// add network window
			case 'saveNetwork':
				shell_exec($wicd_root . 'shell.sh wicd saveNetwork "' . $_GET['networkName'] . '" "' . $_GET['networkPassword'] . '"');
				echo 'true';
				exit();
			break;
			...
			// network switches
			case 'connect':
				shell_exec($wicd_root . 'shell.sh wicd connect "' . $_GET['network'] . '"');
				echo 'true';
				exit();
			break;
			case 'setAutoconnect':
				echo shell_exec($wicd_root . 'shell.sh wicd setAutoconnect "' . $_GET['network'] . '" ' . $_GET['state'] . ' "' . $wicd_root . '"');
				exit();
			break;
